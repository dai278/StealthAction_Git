...
void AEnemyBase::UpdateSearch(float _deltaTime)
 {
	...
	else if (m_cautionPriorityCheck)
	{
	...
	else if (m_visionCheck)
	{
		...
		//帰還の場合
		else if (m_visionLevel == 8 || m_visionLevel == 9)
		{
...
void AEnemyBase::ResetStateValues(float _deltaTime)
{
	...
	if (m_enemyCurrentState == EEnemyStatus::Return || m_enemyCurrentState == EEnemyStatus::Patrol)
	{
		m_discoveryTime = 0;
	}
...
void AEnemyBase::CaseDoubt(float _deltaTime)
{
	...
	m_discoveryTime += m_deltaTime;			//発見時間の経過

	//時間が経過すると注意状態に移行
	if (m_doubtTime > m_doubtTime_Limit)
	{
		m_visionLevel = 3;		//注意に移行
