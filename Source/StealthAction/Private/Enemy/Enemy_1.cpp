...
void AEnemy_1::CasePatrol(float _deltaTime)
{
	...
	if (m_pEnemy_Route.Num() == 0)
	{
		...
		{
			//レイを飛ばし、全てのオブジェクトに対してコリジョン判定を行う
			bool isHit = GetWorld()->LineTraceSingleByObjectType(HitCollision, m_enemyPos, Enemy_1Forward_Pos, FCollisionObjectQueryParams::AllObjects, CollisionParams);
		...
		if (!m_patrol_TurningCheck)
		{
			...
			{
				//レイを飛ばし、全てのオブジェクトに対してコリジョン判定を行う
				bool isHitXY = GetWorld()->LineTraceSingleByObjectType(HitCollision, m_enemyPos, m_enemyPos_XY_Wall[i], FCollisionObjectQueryParams::AllObjects, CollisionParams);
