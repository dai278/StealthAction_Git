...
void AEnemy2::CaseBattle(float _deltaTime)
{
	...
	//移動する位置、視点の指定
	m_playerPos_LastSeen = m_noise_Pos_keeper;

	//視点移動処理
	UpdateViewMove(_deltaTime);

	FVector2D NoisePos(m_noise_Pos_keeper.X, m_noise_Pos_keeper.Y);
	FVector2D EnemyPos(m_enemyPos.X, m_enemyPos.Y);


	double distance_2D = (NoisePos - EnemyPos).Length();	//音との距離を測る(Vectorの長さ）

	double distance = (m_noise_Pos_keeper - m_enemyPos).Length();	//音との距離を測る(Vectorの長さ）
