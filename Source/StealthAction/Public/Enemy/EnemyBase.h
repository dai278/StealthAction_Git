...
public:
	...
	void OnNoiseHeard(const int& _noiseVolume, const FVector& _pos);
		...
		UPROPERTY(EditAnywhere, Category = "Time")
		double m_notFoundTime_Limit;			//未発見時間制限

		UPROPERTY(EditAnywhere, Category = "Time")
		double m_notFoundNoiseTime_Limit;			//物音未発見時間制限
		...
		double m_notFoundTime;				//物音未発見時間
		double m_notFoundNoiseTime;			//物音未発見時間
		...
		bool m_cautionPriorityCheck;		//注意優先状態か
